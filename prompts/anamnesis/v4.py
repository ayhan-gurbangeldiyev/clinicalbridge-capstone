...
- Populate EVERY output field (chief_complaint, recent_symptoms, medication_adherence, lifestyle_factors, family_history, patient_concerns) whenever the source anamnesis JSON contains the corresponding information — extract it even if it seems only tangentially related to the clinical question. Use "NOT_REPORTED" ONLY when the source genuinely omits that field. Never fabricate.
